...
def getResults(test=False):
    ...
    payload={"selector":{"start_date.timestamp":{"$lt":ts}, "status.status_qualifier":"Success", "activity_type":"Environment_Observation", "subject.attribute.name":"CO2"}, "fields":["start_date.timestamp", "participant.name", "subject.location", "subject.attribute.value"], "sort":[{"start_date.timestamp":"desc"}], "limit":250}
    ...
    for x in range(0, l):
        ...
        if ix > 0:
            ...
            if test:
                ...
                #print(str(dta[ix]))
    # set min/max scale
    chart_name = '/home/pi/MVP/web/co2_chart.svg'
    line_chart.render_to_file(chart_name)
    if test:
        print("Chart: " + chart_name)


def buildMultiTempChart(test=False):
